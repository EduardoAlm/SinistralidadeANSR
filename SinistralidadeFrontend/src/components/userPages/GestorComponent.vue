  ...
  <div class="w3-container">
   ...
   overflow:auto;">
      ...
      <table class="w3-table w3-bordered w3-centered w3-striped">
        ...
        <tr v-for="ac in acidentes" v-bind:key="ac.id" class="w3-hover-light-gray" @click="selectAcidente(ac.id, ac.concelho, ac.ocupacao, ac.datahora, ac.mortos, ac.feridosg, ac.via,ac.km, ac.natureza)">
          <td>{{ ac.id }}</td>
          <td>{{ ac.concelho }}</td>
          <td>{{ ac.datahora }}</td>
          <td>{{ ac.mortos }}</td>
      ...
      <p>&nbsp;</p>
      <button class="w3-button w3-green w3-round w3-margin-right" @click="createAcidente">Criar</button>
      <button class="w3-button w3-green w3-round" @click="updateAcidente">Atualizar</button>
    ...
    <div class="component-container" style="width: 100%">
      <table class="w3-table w3-bordered w3-centered w3-striped">
        ...
        <tr v-for="user in this.users" v-bind:key="user.id" class="w3-hover-light-gray" @click="selectUser(user.cc, user.nome, user.ocupacao, user.palavrapasse, user.n_distrito)" >
          <td>{{ user.cc }}</td>
          <td>{{ user.nome }}</td>
          <td>{{ user.ocupacao }}</td>
      ...
      <modal name="atualizarUtilizadorModal" height="auto" :scrollable="true">
        ...
        <div class="w3-container w3-row">
          <div class="w3-cell-row">
            <h5 class>Nome de Utilizador</h5><input
            :id="userName"
            v-model="userName"
            @keyup="checkNull"
...
export default {
  ...
  methods: {
    selectAcidente(id, concelho, ocupacao, datahora, mortos, feridosg, via, km, natureza){
      this.id = id;
    ...
    modalDeleteUser: function() {
      this.$modal.show("apagarUtilizadorModal");
    },
